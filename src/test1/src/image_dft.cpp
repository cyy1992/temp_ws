...
ImageDFT::ImageDFT(int rows, int cols)
{
  ...
  forward_plan_ = fftw_plan_dft_2d(
      rows_, cols_,
      const_cast<fftw_complex*>(reinterpret_cast<fftw_complex*>(in_.data())),
      const_cast<fftw_complex*>(reinterpret_cast<fftw_complex*>(out_.data())),
      FFTW_FORWARD, FFTW_MEASURE);
  backward_plan_ = fftw_plan_dft_2d(
      rows_, cols_,
      const_cast<fftw_complex*>(reinterpret_cast<fftw_complex*>(iin_.data())),
...
ComplexMatrix ImageDFT::fftShift(const ComplexMatrix& in)
{
  ...
  out.block(0, 0, block_rows, block_cols) =
      in.block(block_rows, block_cols, block_rows, block_cols);
  out.block(block_rows, block_cols, block_rows, block_cols) =
      in.block(0, 0, block_rows, block_cols);
  // swap second and fourth quadrant
  out.block(block_rows, 0, block_rows, block_cols) =
      in.block(0, block_cols, block_rows, block_cols);
  out.block(0, block_cols, block_rows, block_cols) =
      in.block(block_rows, 0, block_rows, block_cols);
  return out;
}
...
ComplexMatrix ImageDFT::crossPowerSpectrum(const ComplexMatrix& f1,
                                           const ComplexMatrix& f2)
{
  // (f1 x f2*) / (abs(f1) x abs(f2))
  double eps = 1e-15; // add eps in case denominator is zero
  Eigen::MatrixXd denom =
      f1.cwiseAbs().cwiseProduct(f2.cwiseAbs()).array() + eps;
  ComplexMatrix out = (f1.cwiseProduct(f2.conjugate())).cwiseQuotient(denom);
  return out;
}

void ImageDFT::phaseCorrelate(const cv::Mat& im0, const cv::Mat& im1,
...
void ImageDFT::getCentreOfMass(const Eigen::MatrixXd& f1, double& row,
                               double& col)
{
  Eigen::VectorXd cols =
      Eigen::VectorXd::LinSpaced(f1.cols(), 0, f1.cols() - 1);
  Eigen::VectorXd rows =
      Eigen::VectorXd::LinSpaced(f1.rows(), 0, f1.rows() - 1);
...
Eigen::MatrixXd ImageDFT::neighbourhood(const Eigen::MatrixXd& f1,
                                        ...
                                        int radius)
{
  ...
  // FIXME: col_end should be used?
  // if (row_start > 0 && row_end < f1.rows() && col_start > 0 && col_start <
  // f1.cols())
  if (row_start > 0 && row_end < f1.rows() && col_start > 0 &&
      col_end < f1.cols())
  {
