...
Kalman::Kalman()
{
  ...
  setIdentity(kf_.processNoiseCov, Scalar::all(0.5));
  setIdentity(kf_.measurementNoiseCov, Scalar::all(1));
...
double Kalman::Var()
{
  ...
  return var_sum/distance_queue_.size();
}
...
//   ;
  last_distance_ = distance;
  return (double)(kf_.correct(measurement_).at<float>(0));
}

UwbFilter::UwbFilter(const ros::NodeHandle& n):nh_(n)
{
  uwb_sub_ = nh_.subscribe<nlink_parser::LinktrackNodeframe2>(
    "/nlink_linktrack_nodeframe2", 5000, &UwbFilter::HandleUwbMsg,this);
